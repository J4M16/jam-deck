...
- 一个有意思的发现：`body[data-jam-deck-skin="glass"] .jam-deck-task-field { border: 0; padding: 0; }` 早就存在——**玻璃皮肤按规范做过了，纸面皮肤一直没跟上**。所以这次不是发明新形态，是把纸面对齐到既有的正确形态。
- 详情弹窗：字段去卡片，靠 20px 留白与低对比标签分区。标签色原本直接用 `--jd-muted`，实测计算值是 `rgb(92,92,92)`——当前主题的 `--text-muted` 恰好等于标题固定的 `#5c5c5c`，层级整个塌掉，于是再降一档透明度。保存按钮原本挂 Obsidian 的 `mod-cta`（紫色），按规范改成 `--jd-ink` 底、纸色字的黑白主按钮；「完成并归档」原本用 `--interactive-accent` 填充，降为描边，避免两个实心按钮抢焦点。输入框聚焦色与字段焦点环从主题紫统一换成品牌绿。
- 归档弹窗：取消逐行卡片，改行透明 + 发丝分隔 + 悬停低对比底色；「恢复 / 彻底删除」改为仅在行 `:hover` / `:focus-within` 时出现，与剪贴板操作条的既定做法一致；底部「清空全部归档」不属于任何行，单独覆盖回常驻描边（否则会被 `opacity: 0` 一起藏掉）。补上了它一直缺失的 modalEl 类 `jam-deck-archive-modal-shell`，并把两个弹窗都纳入与 Canvas 选择、快捷方式编辑共用的底材块，该块从此只管调色板与表面，尺寸内边距各自声明。
- **两个 Obsidian 原生按钮的坑**：① 主题用 **inset `box-shadow`** 画按钮外框，我把 `background` 和 `border` 都清了，白框依旧在——必须显式 `box-shadow: none`；② 原生按钮是 **flex 容器**，`text-align: left` 计算值确实是 `left` 却依然居中，需要 `display: block` 才生效。两条都写进了规范。
- **验证工具再次踩坑，记录清楚**：① `capturePage` 对**相同的裁剪 rect 会返回缓存帧**——改完 CSS 后连截两张内容完全一致，误以为改动没生效；把 rect 挪动几像素即可拿到新帧。② 用 `modalContainer.remove()` 关闭 Modal 会留下孤儿 `.modal-container`，累积到 3 个，导致 `document.querySelector(".modal-container .modal")` 取到的是最早那个孤儿，于是「打开归档弹窗却截到待办详情」。正确做法是取最后一个容器，或干脆别用 remove——普查阶段我也用过 remove 关弹窗，那批判定结果可能受类名残留影响，后续重跑普查时需留意。
- 本轮结论仍以计算样式为准：`display: block`、`box-shadow: none`、`text-align: left`、背景透明均已确认生效，截图只作最终观感确认。
...
- 方法两遍。第一遍「清空」：临时把规则的 `cssText` 置空，只重读该规则声明过的那些属性的计算值，无变化说明它没有贡献。第二遍「哨兵」：塞一个不可能的值（颜色用 `rgb(1, 2, 3)`、长度用 `12345px`、`display` 用 `inline-table`），看计算值是否跟随。只有两遍都失败才判定为「被覆盖」。第二遍是必需的——`--jd-heading` 与当前主题的 `--text-muted` 恰好都是 `#5c5c5c`，只做第一遍会把生效的标题色规则误判成死代码。实测中该规则确实被正确归入 `redundant` 而非 `overridden`。两遍都在 `finally` 里还原规则，不落盘。
- 踩到一个 CSSOM 陷阱值得单独记：遍历样式表时写成 `if (rule.cssRules) { walk(rule.cssRules); continue; }` 会**漏掉全部普通规则**。Chrome 支持 CSS 嵌套后，普通 `CSSStyleRule` 也带一个（通常为空的）`cssRules` 列表，对象恒为 truthy，于是每条普通规则都走了 `continue`。正确写法是先收集再递归，且递归条件加上 `.length`。这个 bug 让前两次统计返回 0 条，差点得出「样式表不可访问」的错误结论。
- 另一个坑：脚本把探针压成单行供 CLI `eval` 使用，压行会让 `//` 行注释吃掉其后所有代码。探针内一律使用块注释，并在输出前断言不含单引号，保证生成的命令可以安全地单引号包裹。
- 首次运行结果（仅打开工作台）：1022 条 jam-deck 规则，**覆盖率 26%**——554 条在当前 DOM 无匹配元素（弹窗、Canvas、灵动岛、字幕墙均未打开），198 条是状态伪类，实际判定 270 条。其中 29 条确证被覆盖，5 条属值与继承巧合相同的冗余声明。被覆盖名单集中在 `.jam-deck-clip-*`、`.jam-deck-drawing-palette`、`.jam-deck-launcher-icon.is-domain`、若干 `.jam-deck-root .jam-deck-ai-*` 与 `.jam-deck-music-*`，以及 `.jam-deck-toolbar`。
- **本次不执行删除**。26% 覆盖率下动手不负责任：未被测到的规则不等于死，只是没机会判定。后续应先把弹窗、内嵌 Canvas、灵动岛、字幕墙逐一打开再重跑，把覆盖率顶上去，再按名单统一清理。脚本头部已写明这条纪律。
- 工具：WorkBuddy；处理模型签名：具体模型标识不可见（主代理、审查与实现）。

## 2026-09-26 — 1.2.0 视觉规范去双份真相

- `docs/VISUAL_DESIGN.md` 此前同时承担规范与变更日志两个职责：规则正文里嵌着「1.1.2 起」「1.1.3 起」「1.1.8 按」「当前 0.32.12」等版本前缀，读者需要先判断某条是否仍然适用；同时又把 `--jd-radius-sm`「当前 10px」、`--jd-radius-md`「当前 14px」的像素值抄了一份，与 `styles.css` 构成双份真相，必然漂移。本次移除全部版本前缀与重复数值，规范只描述现状并引用令牌名，历史归 CHANGELOG。
- 新增「设计令牌」一节，把本批实测得到的规则固化下来，避免后续重复踩坑：令牌唯一来源；作用域规则（弹窗挂在 `.jam-deck-root` 之外，共用令牌必须定义在 `body` 块，且该块只放跨主题不变的值）；角色优先于尺寸；图标圆角是等比关系、不与阶梯归并；给原生表单元素写样式必须带元素类型；改 `.jam-deck-*` 基础规则前先确认有无 `.jam-deck-root` 重写版本，结论以运行中计算样式为准。
- 纯文档改动，不涉及代码。工具：WorkBuddy；处理模型签名：具体模型标识不可见（主代理、审查与实现）。

## 2026-09-26 — 1.2.0 设计令牌统一（第 1 批：色值与仓库卫生）
...
- 修复玻璃两处闪烁：灵动岛将整个滤镜输出保持透明，首帧桌面画好后才显示，收起同步隐藏，避免未就绪画布被滤镜填成黑底；工作台勾选或重绘时保留连接中的背景图片/视频、解码器和明暗配色，只重建控件及对应观察器。补充旧窗口延迟关闭不能影响新灵动岛的保护。npm run verify 全绿，新增首帧显示、背景连续性与旧窗口关闭回归；Windows 抓帧确认首帧前由不透明黑底恢复透明，倒计时/待办勾选验证背景零移除、零重新加载，个人设置未改写。1.1.9 已部署热重载，Mac 共用渲染修复但 M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-21 — 1.1.8 灵动岛桌面折射

- 灵动岛改为持续桌面采样与工作台同款 Hyalite 边缘折射，共用现有 0–16 模糊及均衡/轻盈设置，不新增调节项；保留平顶、31px 下圆角、透明控件，无 40% 白色填色。Windows 排除自身后采集桌面视频，Mac ScreenCaptureKit 显式排除自身并裁切目标区域，最高 30fps、无音频、不落盘或上传；收起停止，退出/卸载清理。npm run verify 全绿；Windows 实机确认条纹边缘折射、模糊 0/4/16、轻盈档、透明填色、10px 收起与轨道停止、重新展开、返回工作台及热重载清理。Mac Apple Silicon 构建成功并嵌入标准三文件，M5 权限、视觉和性能未实机验证。1.1.8 已部署热重载，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-21 — 1.1.7 灵动岛圆角磨砂

- 构建工具：新增 Apple Silicon 桌面区域采样的构建源与 macOS CI，使用 ScreenCaptureKit 显式排除灵动岛自身、限制 30 fps、仅传输目标区域且不采集音频；本提交仅提供构建工具，尚未接入运行界面，现有 1.1.7 不变。npm run verify 全绿，Mac M5 未实机验证。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

- 灵动岛接入独立原生桌面磨砂，保留平顶与 31px 下圆角；按最新要求移除 40% 白色填色，仅保留轻高光和透明控件。Windows 使用 Composition 几何裁切，macOS 实现 AppKit maskImage；原生层不接收输入，收起隐藏，退出、卸载和异常时清理并恢复工作台，标准三文件安装完整携带材质实现。npm run verify 全绿；Windows 150% 双屏实机确认真实模糊、圆角外无矩形底块、透明填色、10px 收起、返回工作台、材质异常恢复与热重载清理，新增进程常驻样本约 15.5 MiB。1.1.7 已部署热重载，Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-21 — 1.1.6 灵动岛白色底层

- 灵动岛展开态增加 40% 白色底层，保留方向高光、31px 下圆角与无桌面磨砂的透明窗口；仅调整表面底色，不改交互及工作台。npm run verify 全绿，Windows 实机计算样式确认白色 alpha 0.4、圆角 31px、无模糊，窗口截图已检查。1.1.6 已部署热重载；Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-21 — 1.1.5 灵动岛清透玻璃圆角

- 修复灵动岛展开后系统玻璃底层露出矩形边角的问题。按 Jam 确认选择，恢复原有平顶与 31px 下圆角，改为无桌面磨砂的清透玻璃，保留透明底色、轻高光与原有控件；移除 acrylic/vibrancy 和窗口区域拼接，工作台玻璃及「映画」入口不变。npm run verify 全绿，Windows 150% 缩放的桌面截图及两侧角外像素确认无矩形底块，实机确认 10px 收起、72px 展开和返回工作台；回归覆盖点击穿透与尺寸同步。1.1.5 已部署热重载，Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-21 — 1.1.4 灵动岛系统玻璃与映画入口

- 灵动岛随玻璃皮肤使用系统材质，透出桌面和下方应用；Windows 使用 acrylic，macOS 使用 under-window vibrancy，统一透明控件、轻边缘与方向高光。展开仅占 72px 可见表面，收起立即释放系统模糊并裁到 10px 细条，保留点击穿透及返回工作台；不捕获桌面、不复制壁纸视频，模糊强度由系统控制。顶栏新增无底文字「映画」，共用图片/视频导入入口，纸面下成功导入后启用玻璃。npm run verify 全绿，Windows 12 项实机检查、桌面合成截图与 Chromium 文件选择请求验证通过，保留用户背景；两平台材质分支通过模拟回归，Mac M5 未实机验证。1.1.4 已部署热重载，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-20 — 1.1.3 Canvas 焦点与原生笔记配色

- 修复玻璃 Canvas 激活描边过亮：普通焦点使用与静止状态相同的细边，保留布局编辑虚线与碰撞提示。原生 Canvas 内容边界恢复 Obsidian 的正文色，笔记预览、编辑和继承文字不再跟随深壁纸变白，玻璃导航仍自适应。仅修改 CSS；npm run verify 全绿，Windows 28 项检查通过（含隔离 DOM 的明暗主题 × 深浅壁纸组合），真实五个文字节点均已渲染，白底深字与激活细边截图确认，Canvas 数据和节点对象保留。1.1.3 已部署热重载；Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证、控件审查子代理）。

## 2026-09-20 — 1.1.2 玻璃控件与壁纸亮度自适应

- 统一玻璃控件：倒计时输入与翻牌改为轻透明表面，普通按钮去除重复底板与阴影，收轻待办悬停、删除按钮和分隔线，移除重复标题的黑色提示；字幕/AI 选中态改用细线，自有表单与键盘焦点同步整理。新增整张壁纸平均亮度自适应：深背景浅字、亮背景深字，含背景压暗与透明像素；图片解码后取样，视频每秒以 32×18 缩略图读取，明暗缓冲区防闪，暂停/隐藏/卸载停止，替换媒体隔离旧回调。Canvas 原生工具栏及空状态文字同步配色，节点内容保留。npm run verify 全绿；Windows 37 项控件检查及 11 项亮度检查通过，真实 4K 视频取样耗时 1.7/2.0/4.9ms（3 次短样本，非帧率保证）。1.1.2 已部署热重载，用户背景与数据保留；Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证、控件审查子代理）。

## 2026-09-20 — 1.1.1 玻璃细节与原生界面开关

- 细化 Liquid Glass：顶栏四按钮改为无底纯文字，移除 Canvas 右侧双层底块，内部分割线与外沿同强度，清除原生底栏 SVG 白填充；弱化白边并增强边缘折射，Canvas 材质独立分层避免底栏折射采样变白。新增 0–16 模糊实时预览（松手保存、复用贴图），纸面/玻璃共用侧栏和顶栏独立隐藏开关，隐藏顶栏时一并隐藏三个窗口按钮，点「恢复界面」后恢复，切笔记及卸载恢复。npm run verify 全绿；Windows 30 项实机检查、明暗截图、原生节点保留及真实设置滑块通过，默认/最大模糊短样本中位间隔 32.7/32.8ms，不代表本地高刷或功耗结果。1.1.1 已部署热重载，用户背景保留；Mac M5 未实机验证，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证、方案调研子代理、架构审查子代理）。

## 2026-09-20 — 1.1.0 Liquid Glass 皮肤与媒体背景

- 新增可在设置切换的 Liquid Glass 玻璃皮肤，组件框、工具栏、按钮和自有弹窗统一通透材质，Canvas 仅背景透底；集成 MIT Hyalite 0.5.0，按窗口隔离并完整释放资源，最多 4 个/合计 36 万 CSS 像素的单通道折射表面，其余使用轻玻璃。支持库内图片/视频背景、压暗、均衡/轻盈档与视频暂停；单媒体实例，隐藏、灵动岛和减少动态效果时暂停；皮肤原地切换，保留 Canvas/AI/字幕节点。补齐失败回退、导入竞态、跨文档及卸载保存队列保护。npm run verify 全绿；Windows Obsidian 实测图片/1080p30 视频、真实设置切换、暂停恢复、明暗样式和跨文档清理通过，远程桌面短样本的帧间隔与原皮肤均约 31ms，不作 60/120fps 保证；Mac M5 未实机验证。1.1.0 开发版已部署热重载，测试附件已清理，未发布正式 Release。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证、方案调研子代理、架构审查子代理）。

## 2026-09-17 — 1.0.2 移除重复叠层按钮
...
## 2026-09-17 — 1.0.1 Canvas 文件夹按钮修复
...
## 2026-09-16 — 0.32.16 快捷方式间距与自适应居中
...
- 快速验证：仅三处颜色声明调整，标题使用 --jd-muted、组件图标使用 85% muted + 15% surface，随主题变量适配明暗；当前浅色主题标题为 rgb(92,92,92)，图标约 rgb(116,116,116)。实际用户字号设置保持不变，部署保护个人数据；四份变更记录逐条核对签名。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-16 — 0.32.13 全局字号与字幕独立字号

- 先读取 GLM 0.32.12 的最新提交，保留网页六色、快捷方式文件夹专属配色、杏→藕粉渐变及 app-window 图标。新增界面小 / 中 / 大和字幕与跟读跟随 / 小 / 中 / 大两项设置；当前样式作为小档，已有设置缺少字号时保持小档，新安装默认中档。自有文字改用语义 CSS 变量，中大档调整控件高度、换行与网格；图标、时钟主数字、Canvas 内容与网页排版保持原有尺寸规则。主窗口、弹出窗口和灵动岛同步应用，更新无需重建工作台；设置写入串行化，失败恢复原值，卸载清理自有变量。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 验证：标准级，npm run verify 全绿；新增默认档位、保存回读、失败回退、快速连续切换、跨文档应用、独立字幕和清理回归。Windows Obsidian 小档实测与 0.32.12 一致，待办正文 12/14/16px、快捷方式名称 9/12/14px，时钟主数字始终 58px；字幕跟随及独立设置 17/20/24px 生效，原组件节点保留。实际下拉事件、340px 窄窗和大字号弹窗无横向溢出；独立 Electron 隐藏窗口验证真实灵动岛渲染，芯片正文随档位变化、时钟数字不变。0.32.13 已部署热重载，隔离 QA 后恢复用户小档并清理临时窗口；Mac M5 未实机验证，无新增平台依赖。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
...
- 验证：npm run verify 全绿；Windows Obsidian 隔离弹窗实测宽 500px 时字符模式高约 409px，标题距顶部约 25px（原约 44px），名称 120px、链接约 313px，预览/外观相邻、名称/链接同排、提示/按钮中心线一致；340px 宽度无横向溢出，名称与链接自动换行。已截图确认并部署热重载，四份记录签名核对；本次仅 DOM/CSS 排版，无新增平台依赖，Mac M5 未实机验证。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-16 — 0.32.10 快捷方式封面与编辑弹窗

- 沿用 baoyu-design 的既有设计系统复用与实机预览流程：弹窗单纸面、名称/路径标签、黑白主按钮、轻量外观切换与统一极细滚动条。支持完整字素字符封面、双色渐变及自动深浅字色；默认应用图标/网页域名图标保留，文件夹复用 Canvas SVG 壳体路径、叠纸/磨砂前片/双凹槽与六色配色。预览与列表共用图标渲染，外观作为独立 appearance 字段保存；取消不落盘，失败保留输入可重试，忽略旧图片迟到的加载失败事件。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 验证：npm run verify 全绿；新增字素/颜色校验、序列化回读、草稿隔离、保存失败回退、重复链接、图片迟到事件及 Windows/macOS 应用与文件夹目标回归。Windows Obsidian 内使用隔离设置验证中文输入过程、组合 emoji、取消、保存/重新打开及失败后重试；截图确认字符与文件夹两种界面，检查六色切换、340px 窄窗、390px 限高滚动、默认可见 2px 滑块及深色主题计算样式。0.32.10 已部署热重载，个人 data.json 未通过部署改动；临时弹窗已关闭。使用共享 DOM/CSS，无新增包或平台 API；macOS Apple Silicon M5 尚未实机验收。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-16 — 0.32.9 Canvas 弹窗滚动条规范补修

- 原因：既有极细滚动条只匹配 .jam-deck-root，原生 Modal 挂在根节点外未命中。将 Canvas 选择弹窗加入同一组 scrollbar/track/corner/button/thumb/hover 选择器，不复制另一套数值、不覆盖其他 Obsidian 弹窗；把历史变更中的精确规格补入 VISUAL_DESIGN。已确认本地 baoyu-design 可用，本次参考其复用既有设计上下文及实际预览检查流程。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 验证：npm run verify 全绿；0.32.9 部署热重载，Windows 真实弹窗计算样式为占位 6px、两侧透明边各 2px、滑块强度 20%、轨道透明、箭头 none、标准 scrollbar-width 为 auto，实际 scrollTop=120，截图确认极细滑块。悬停直接复用已存在的 6px / 40% 规则；Mac M5 尚未实机验收。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-16 — 0.32.8 Canvas 选择弹窗与新建入口

- 选择器采用独立作用域的 Spatial 纸面样式，修复原按钮固定行高导致名称/路径拥挤。文件项改为左对齐图标与双行文字、细分隔、当前状态小点；标题右侧增加黑白高对比的新建主按钮，搜索区域与列表计数分层，空结果保留新建入口，支持方向键选择及原生 Enter/Space 激活。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 新建复用既有未命名 Canvas 编号规则，使用 Vault 相对路径写入标准空白 JSON，再走原有组件添加/更换入口；所有选择统一防重复点击。保存成功但放置失败时保留文件并提示路径，重试复用该文件；创建途中关闭弹窗不再切换组件。未新增平台依赖或修改 Canvas 挂载生命周期。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 验证：npm run verify 全绿，新增已有选择、新建替换/插入、不重名、防连击、磁盘失败、绑定失败/重试、关闭中途创建回归。Windows 实机检查搜索空态、键盘定位、11 条列表名称与路径不重叠、当前标记唯一；实际点击新建写入空白 Canvas，通过隔离组件设置调用真实更换/插入方法，并成功挂载到原生 Canvas。浅色截图确认样式；深色主题计算样式确认纸面与主按钮反转。临时画布/伴随文件已清理，主题恢复。0.32.8 已部署热重载；macOS Apple Silicon M5 尚未实机验收。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。

## 2026-09-16 — 0.32.7 字幕墙紧凑尺寸

- 新建默认 18×18 → 10×9，最小完整显示 10×14 → 6×8。沿用统一网格缩略判定，已有组件可继续手动缩小；不重写既有布局或字幕草稿。减小内部间距、正文上下 padding 与段间距，保留 17px 正文和 32px 操作按钮。跟读空闲说明不再占底栏，实际语音保持单行省略并提供完整 title。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
- 验证：npm run verify 全绿。Windows Obsidian 内隔离 UI 实测约 320×280 小窗，两行字幕需要 70px，转录可用正文 153px、跟读显示语音底栏与手动滚动按钮时可用 91px；无外层溢出，截图确认两行完整可见。试验 7 格高跟读正文不足后，将正式门槛定为 8 格；临时 UI 已清理。使用共享 DOM/CSS，无新平台依赖；macOS Apple Silicon M5 尚未实机验证。工具：Codex；处理模型签名：具体模型标识不可见（主代理、实现与验证）。
...
- Jam：胶囊折叠延时改为设置面板配置。
- 新增 `islandLeaveMs`（0.3–3 秒下拉，默认 1000ms）；离开轮询与岛内 leave 计时共用 `getLeaveMs()`。
- 验证：快速。差异审查、`npm run verify`、部署热重载。
- 处理模型签名：Cursor Grok 4.6（主代理）

## 2026-08-31 — 0.31.41 修复灵动岛离开不折叠

- Jam：鼠标移走后胶囊不收起。
- 根因：离开折叠只靠 DOM `mouseleave`，透明阴影垫仍算在窗口内；`pointermove` 还会清掉 leave 计时。改为 `getCursorScreenPoint` 对照可见胶囊矩形轮询，阴影区视为已离开。
- 验证：标准。`npm run verify`、部署热重载实机移出收起。
- 处理模型签名：Cursor Grok 4.6（主代理）

## 2026-08-31 — 0.31.40 灵动岛折叠点击穿透

- Jam：折叠态虽只有 10px 白条才展开，但窗口仍挡住下方浏览器标签，点不到。
- 根因：折叠窗仍约全宽 × 24px（含阴影垫），透明区也吃点击。改为折叠即刻 `setIgnoreMouseEvents(true, { forward: true })`，morph 后窗口收到 10px × 70% 白条；展开时关闭穿透。
- 验证：标准。差异审查、`npm run verify`、部署热重载实机点浏览器标签。
- 处理模型签名：Cursor Grok 4.6（主代理）

## 2026-08-29 — 0.31.39 移除灵动岛 AI 第二页

- Jam：不要灵动岛 AI 第二分页了。
- 删除岛内 page / 滚轮切页 / AI 表单与相关 IPC；灵动岛恢复为剪贴板芯片 + 倒计时 +「工作台」。工作台 AI 会话保留。
- 验证：快速。差异审查、`npm run verify`、部署热重载。
- 处理模型签名：Cursor Grok 4.6（主代理）

## 2026-08-29 — 0.31.38 灵动岛 AI 页布局

- Jam：第二页不美观——模型切换应在输入框内左侧，等距缩小；发送与「工作台」同宽；有字后由灰白变品牌色。
- 输入与模型并入 `.ai-field` 胶囊；发送固定 100px；`is-ready` 亮品牌绿。
- 验证：快速。差异审查、`npm run verify`、部署热重载。
- 处理模型签名：Cursor Grok 4.6（主代理）

## 2026-08-29 — 0.31.37 灵动岛第二页 AI

- Jam：绿点「灵动」上滚轮切到第二分页；该页没有剪贴板和番茄钟，只有 AI 输入框、发送，以及 DeepSeek / 千问切换。
- 岛内 `page` 状态：滚轮下到 AI、上回剪贴板。发送走现有 `JamDeckView.sendAiText`，模型切换走 `toggleAiProvider`，与工作台同一会话和 Key。
- 视觉：第二页仍是 54px 胶囊行，模型用文字按钮，绿色只作输入焦点环。
- 验证：快速。差异审查、`npm run verify`、部署热重载。
- 处理模型签名：Cursor Grok 4.6（主代理）
...
- 验证：标准。差异审查、`npm run verify`、受保护部署与热重载均通过，`data.json` 保持 SHA-256 `232E8F29BE40A9443F723CD117EA7C99730DA5CE1045E210BAA28C0116FE1C9D`（33391 bytes）。正式视觉检查：2× 时壳体 400×300，底板/4 个代表图/磨砂前片/文案全部可见，且一个成员被 virtualize 时壳体仍保留；1× 时 200×150，悬停散开与轻量控制条正常。
...
- 原因：不是文件夹改动冲突。运行时 `.jam-deck-ai-fab` 与聊天 DOM 都存在，但持久位置为 `{x:2444,y:1241.135...}`，当前 Jam Deck 根区域仅约 `1576×954`；根容器 `overflow:hidden`，按钮因此被裁到右下屏幕外。旧代码只在拖动按钮时按当时窗口限制坐标，重载、窗口缩小、换显示器或分栏改变后会直接复用旧坐标；0.30.7 热重载只是重新暴露了这个老漏洞。
- 修复：新增纯位置校正函数；`layoutAiFabChat` 在有效容器尺寸下统一 clamp 按钮坐标，实际发生越界校正时才写回设置。根区域 ResizeObserver 在窗口/分栏尺寸变化后按帧重排，render/close 时完整释放 observer 与 rAF；初始 `0×0` 不覆盖持久坐标，聊天窗仍按校正后的按钮位置限制在可视范围。
- 回归：覆盖 `2444/1241 → 1576×954` 回收、默认右下 20px、零尺寸不覆写以及 resize observer 生命周期；`npm run verify` 全绿，部署热重载后实机确认按钮回到可视区域。
- 处理模型签名：具体模型标识不可见（主代理/实现与实机验证）、具体模型标识不可见（只读原因侦察）

## 2026-08-14 — 0.30.7 恢复文件夹局部推挤并减弱翻动幅度

- Jam 反馈：文件夹展开后原先会推开下方零散 Canvas 内容，0.30.5 后该效果消失；同时希望悬停翻开与收起合拢的动画幅度减弱 40%。
- 原因：这不是合并冲突。0.30.5 为解决 42 节点图片画布的开合卡顿，在 `prepareBystanders` 对 explicit folder 直接返回空数组，有意关闭了文件夹预览的全部周边推挤；同时保留了旧式自由堆叠的避让。
- 恢复策略：文件夹预览重新参与 bystander displacement，但使用 `influence=0` 的窄避让模式，只推开被展开排版实际覆盖的零散节点，并在让位后保留 20px 安全间距；不再推动旧 64px 影响区内只是靠近的节点，并排除隐藏/折叠的其他文件夹成员。继续保留 0.30.5 的全屏滤镜移除、预览 DOM observer 过滤和布局读写分相。
- 动画：悬停角度 `-30deg → -18deg`，WAAPI 与 CSS fallback 的完整开合端点 `-80deg → -48deg`，正好保留原幅度的 60%；perspective、450/600ms 时长和缓动不变。
- 回归：更新 explicit folder 窄避让、隐藏成员排除、48° fallback 一致性与 18° hover 合同；`npm run verify` 全绿。部署到 Jamnote 热重载后，当前四卡文件夹有 9 个实际覆盖节点参与让位，WAAPI 开/合端点分别为 `0→-48deg` 与 `-48→0deg`；GPU-disabled 约 30fps 基线下两次有效冷展开平均帧间隔为 32.7ms / 31.3ms，未重现旧版持续 60ms 以上的卡顿。
- 处理模型签名：具体模型标识不可见（主代理/实现与实机验证）、具体模型标识不可见（只读推挤侦察）、具体模型标识不可见（只读动画侦察）

## 2026-08-13 — 0.30.6 文件夹文案位置微调

- Jam 反馈：文件夹的“4 个节点”和文件夹名称需要整体下移 20px。
- 修复：对折叠文件夹的 metadata 容器统一应用 `translateY(20px)`，两行文案相对间距保持不变；展开态原生文件夹工具条显式清除该位移，代表图、外框、凹槽与交互热区均不受影响。
- 回归：新增折叠文案 20px 位移及展开工具条不位移的样式合同；`npm run verify` 全绿，并部署到 Jamnote 后热重载完成。
...
- 实现：`DEFAULT_SETTINGS.widgets` 改为 Jam 当前 7 组件布局（顶栏 时钟+音乐+快捷方式，左列 待办/日历/剪贴板，右侧 35×30 原生 Canvas）。组件 id 全部稳定化（`music-1`/`launcher-1`/`canvas-embed-1` 等，不带时间戳）；快捷方式列表、Canvas filePath 等个人配置留空——canvas-embed 无文件时走既有「选择一个 Canvas」引导态，不报错。
- 测试 fixture 用自定义 widgets，不依赖默认布局，无需改断言；verify 全绿。
- 处理模型签名：MiniMax-M3（WorkBuddy 主对话）
