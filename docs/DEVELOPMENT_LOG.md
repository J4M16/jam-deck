...
- Canvas AI 回归：实机出现权威 `canvas.selection` 只有 1 个当前文本节点，但 DOM 同时残留 2 个 `.is-selected` 的状态。旧工具栏扫描 DOM class 后误判多选，将 `selectedAiNode` 清空；现在统一以 Canvas selection 集合为唯一事实来源，真实单选正常分类为文本/图片，真实多选才禁用。直接传入 selection 当前节点时，双页助手上下文与消息链已验证正常。
